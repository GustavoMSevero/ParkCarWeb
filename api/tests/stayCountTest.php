...
function stayCount($idClient, $licensePlate, $idParking) {
    ...
    try {
        ...
        $toleranceDay = 'Quarta'; // Dia da Semana de tolerância
        $toleranceTimeDayOfWeek = '01:00'; //Tempo de peranência em dia de tolerancia
        ...
        $time = '02:29'; // Tempo, teste, de permanencia do veículo
        ...
        if ($weekDay == $toleranceDay) {
            ...
            echo $weekDay.' '.$tolerancePeriod."</br></br>";
            echo "Tempo de permanência de teste ".$time."<br></br>";
            if ($time < $tolerancePeriod ) {
                echo 'paga '.$parkingPrice_01."</br>";
                $priceToPay = "0.00";
                echo 'tempo de permanencia '.$time."</br>";
            } elseif ($time > $tolerancePeriod && $time <= $parkingTime_01) {
                echo 'paga '.$parkingPrice_01."</br>";
                $priceToPay = $parkingPrice_01;
                echo 'tempo de permanencia '.$time."</br>";
            } elseif ($time > $parkingTime_01 && $time <= $parkingTime_02) {
                echo 'paga '.$parkingPrice_02."</br>";
                $priceToPay = $parkingPrice_02;
                echo 'tempo de permanencia '.$time."</br>";
            } elseif ($time > $parkingTime_02 && $time <= $parkingTime_03) {
                echo 'paga '.($parkingPrice_03+$adicionalPrice)."</br>";
                $priceToPay = $parkingPrice_03;
                echo 'tempo de permanencia '.$time."</br>";
            } elseif ($time > $parkingTime_03 && $time <= $parkingTime_04) {
